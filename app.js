...
var loginUrl = 'https://accounts.pixiv.net/login?lang=zh_tw&source=pc&view_type=page&ref=wwwtop_accounts_index';
var dataForm = {};
var headers = {
...
var userId = '4726711';
var urlDataArray = [];
var memberUrl = 'http://www.pixiv.net/member_illust.php?id=' + userId;
var jar = request.jar();
...
memberImageList(memberUrl);

function memberImageList(url) {
	console.log('url', url);
	jar.setCookie(rcookie, url);
	request({
		url: url,
		headers: {
			'Referer': 'http://www.pixiv.net/',
		...
		async.map(imageItems, function(data, cb) {
			...
			if (next === undefined) {
				...
				memberImageList('http://www.pixiv.net/member_illust.php' + next);
			}
		});
	});
}
...
function imageDownload(urlDataArray , time) {
	...
	var d = 1;
	var type = false;
	async.mapLimit(urlDataArray, 3, function(urlData, ck){
		if (urlData === null) {
			return ck(null, null);
		}
		async.retry({
			...
			console.log('重試 err :', err);
			console.log('重試 result :', result);
			ck(null, {
				name: urlData.name,
				url: urlData.url
			});
